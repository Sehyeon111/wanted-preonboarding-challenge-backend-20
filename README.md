...
### 고민했던 사항
1. 상품 테이블과 상품 거래 테이블을 어떤컬럼으로 구별해야하나? 거래 현황과 판매자는 상품 테이블에 없어도 될까? 중복 허락? 아님 중복 배제?
2. 로그인한 사용자의 id를 어떻게 가져올 것인가? id를 프론트에서 찾을것인가 아님 controller단에서 찾을것인가?
3. 로그인한 사용자의 거래 목록을 어떤 형태로 보낼것인가?

- 제품 등록과 구매는 회원만 가능합니다. -> 비회원 요청이 들어왔을 경우는? 회원정보가 없는데 등록과 구매 요청이 들어온다면?
- 구매자가 제품의 상세페이지에서 구매하기 버튼을 누르면 거래가 시작됩니다. -> 구매 과정에서 가격의 정보가, 내가 요청한 정보와 동일한지 / 조회 후에 구매하려는 시점에도 판매중인지 / 회원만 구매가능 / 판매자가 구매를 누르는 경우
- 판매자는 거래진행중인 구매자에 대해 '판매승인'을 하는 경우 거래가 완료됩니다. -> 판매 승인을 할 수 있는 상태인지 / 팬매승인이 가능한 구매자인지
